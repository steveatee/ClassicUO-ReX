    ...
    internal sealed class TileMarkerManager
    {
        ...
        public void Save()
        {
            ...
            try
            {
                ...
                var lines = new List<string>(_markedTiles.Count);

                foreach (var entry in _markedTiles)
                {
                    lines.Add($"{entry.Key}={entry.Value}");
                }

                File.WriteAllLines(GetSavePath(profilePath), lines);
            }
            catch
            {
            }
        ...
        private void EnsureLoaded()
        {
            ...
            try
            {
                ...
                foreach (string line in File.ReadLines(savePath))
                {
                    ...
                    if (
                        long.TryParse(line.Substring(0, separator), out long key)
                        && ushort.TryParse(line.Substring(separator + 1), out ushort hue)
                    )
                    {
                        _markedTiles[key] = hue;
                    }
                }
            }
            catch
        ...
        {
            return Path.Combine(profilePath, "tilemarkers.txt");
        }
